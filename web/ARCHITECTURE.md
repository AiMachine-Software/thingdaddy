...
```
   browser  http://localhost:8789
      ...
      ▼
┌─────────────────────────────────────────────────────────┐
│  container: app        (image built from web/Dockerfile) │
│  node:22-alpine · listens 0.0.0.0:8787 · published :8789 │
│                                                          │
│   population/api/server.js  — Express, 26 routes         │
...
│      │      /            → public/index.html             │
│      │      /console.html → public/console.html          │
│      │                                                    │
│      └── 404 fallthrough  {"error":"no route ..."}       │
│                                                          │
│   population/api/db.js — single pg Pool, the ONLY path   │
│                          to the database                 │
└───────────────────────────┬─────────────────────────────┘
...
│  init    web/db-init/10-seed.sh → docker-entrypoint-     │
│                                    initdb.d, FIRST BOOT  │
└─────────────────────────────────────────────────────────┘
```
