...
```bash
cd /Users/p4lmnpk/Desktop/outsource/thingdaddy/code
...
| `Dockerfile` | image เดียว = `population/api` + `web/public` |
...
| `public/index.html` | = `population/fill/td_screens_live.html` แก้ 1 บรรทัด |
| `public/console.html` | = `population/fill/td_console.html` แก้ 1 บรรทัด |
...
1. `web/public/index.html` บรรทัด 649 — `const API = 'http://127.0.0.1:8787';` → `const API = '';`
2. `web/public/console.html` บรรทัด 149 — บรรทัดเดียวกัน เปลี่ยนแบบเดียวกัน
3. `population/api/server.js` — เพิ่ม 5 บรรทัด (import `fs` + เสิร์ฟไฟล์ static แบบ opt-in)
...
`docker compose -f web/docker-compose.yml down` — data survives, because it lives
...
1. `web/public/index.html` line 649: `const API = 'http://127.0.0.1:8787';` → `const API = '';`
2. `web/public/console.html` line 149: the same one-line change
3. `population/api/server.js`: five added lines — an `fs` import and an opt-in
