...
const BASE = (import.meta.env.VITE_API_BASE || (import.meta.env.DEV ? 'http://localhost:8787' : '')).replace(/\/$/, '');
...
async function get(path) {
  ...
  try {
    ...
    throw new Error(`Cannot reach the population API at ${BASE || window.location.origin}. Is it running? (${err.message})`);
  }
  if (!res.ok) {
    let detail = '';
